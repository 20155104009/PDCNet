...
def img_to_patches(img, win, stride=1):
	...
	res = np.zeros([endc, win*win, total_pat_num], np.uint8)
	for i in range(win):
		for j in range(win):
...
##dataset
	...
	with h5py.File(traindbf, 'w') as h5f:
		while i < len(files) and train_num < max_num_patches:
			imgor = np.array(cv2.imread(files[i]),dtype=np.uint8)
			for sca in scales:
				...
				endw1 = img.shape[1]
				endh1 = img.shape[2]
				if endw1 <= patch_size or endh1 <= patch_size:
					#i += 1
					continue

				patches = img_to_patches(img, win=patch_size, stride=stride)
				print("\tfile: %s  # samples: %d" % \
				  (files[i], patches.shape[3] * aug_times))
