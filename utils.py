...
def batch_psnr(img, imclean, data_range):
	...
	for i in range(img_cpu.shape[0]):
		psnr += skimage.metrics.peak_signal_noise_ratio(imgclean[i, :, :, :], img_cpu[i, :, :, :], \
...
def batch_ssim(img, imclean, data_range):
	...
	imclean=imclean.permute(0,2,3,1)

	img_cpu = img.data.cpu().numpy().astype(np.float32)
	imgclean = imclean.data.cpu().numpy().astype(np.float32)
	psnr = 0
	for i in range(img_cpu.shape[0]):
		psnr += skimage.metrics.structural_similarity(imgclean[i, :, :, :], img_cpu[i, :, :, :],data_range=data_range,multichannel=True)
	return psnr/img_cpu.shape[0]
